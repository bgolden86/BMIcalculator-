  ...
  if (tbRecords != null) {
    ...
    for (var i = 0; i < tbRecords.length; i++) {
      ...
      $("#tblRecords tbody").append("<tr>" +
        ...
        "  <td>" + rec.height + "</td>" +
        "  <td>" + rec.weight + "</td>" +
		"  <td>" + (rec.weight/Math.pow(rec.height,2)) + "</td>" +
...
function checkRecordForm() {
  ...
  console.log(Number($("#txtHeight").val()));
  console.log(Number($("#txtWeight").val()));

  if (($("#txtHeight").val() != "") &&
	($("#txtWeight").val() != "") &&
	(Number($("#txtHeight").val()) > 0) &&
	(Number($("#txtWeight").val()) > 0) &&
    ($("#datExamDate").val() != "") &&
